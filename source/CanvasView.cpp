...
void
CanvasView::ScreenChanged(BRect /* frame */, color_space mode)
{
...
void
CanvasView::setScale(float s, bool invalidate)
{
	BRect frame;
...
rgb_color
CanvasView::getColor(long x, long y)
{
...
void
CanvasView::plot_alpha(BPoint p, rgb_color c)
{
	// Needs:
...
void
CanvasView::fplot(BPoint p, rgb_color c)
{
	// Needs:
	//	 sbptr -> pointer to currentLayer()->Bits();
	//   sblpr -> currentLayer()->BytesPerRow()/4;
...
void
CanvasView::fplot_alpha(BPoint p, rgb_color c)
{
	// Needs:
	//	 sbptr -> pointer to currentLayer()->Bits();
	//   sblpr -> currentLayer()->BytesPerRow()/4;
...
void
CanvasView::tplot32(BPoint p, rgb_color c)
{
...
void
CanvasView::tplot8(LPoint p, uchar c)
{
...
void
CanvasView::addLayer(const char* name)
{
	DetachCurrentLayer();
...
void
CanvasView::insertLayer(int index, const char* name)
{
	DetachCurrentLayer();
...
void
CanvasView::translateLayer(int index)
{
	extern Becasso* mainapp;
	mainapp->setMover();
...
void
CanvasView::rotateLayer(int index)
{
	extern Becasso* mainapp;
	mainapp->setRotator();
	fRotating = index + 1;
...
void
CanvasView::do_translateLayer(int /*index*/, int32 mode)
{
	extern Becasso* mainapp;
	SetupUndo(mode);
	uint32 buttons;
...
void
CanvasView::do_rotateLayer(int /*index*/, int32 mode)
{
	extern Becasso* mainapp;
	SetupUndo(mode);
	uint32 buttons;
...
void
CanvasView::flipLayer(int hv, int index)
{
...
void
CanvasView::moveLayers(int from, int to)
{
	if (from == to) {
...
void
CanvasView::do_removeLayer(int index)
{
	///////////////////////////
	//
	snooze(100000);
...
void
CanvasView::makeCurrentLayer(int index)
{
...
void
CanvasView::checkMetaLayer()
{
	myWindow->posview->SetTextLayer(false);
}

void
CanvasView::mergeLayers(int bottom, int top)
{
	fIndex1 = bottom;
	fIndex2 = top;
	SetupUndo(M_MERGE);
...
void
CanvasView::setChannelOperation(int index, int newmode)
{
...
void
CanvasView::setGlobalAlpha(int index, int ga)
{
...
void
CanvasView::setLayerContentsToBitmap(int index, BBitmap* map, int32 resize)
{
	clock_t start, end;
	start = clock();
...
void
CanvasView::resizeTo(int32 w, int32 h)
{
	CloseOpenAddons();
	SetupUndo(M_RESIZE);
...
void
CanvasView::ResizeToWindow(uint32 what)
// 'rwkr', 'rwar'
{
	BRect rect = Parent()->Bounds();
	rect.left = int(rect.left / fScale + 0.5);
...
void
CanvasView::RotateCanvas(uint32 what)
// 'r90', 'r180', 'r270'
{
	CloseOpenAddons();
...
void
CanvasView::ReplaceCurrentLayer(BBitmap* newlayer)
{
...
void
CanvasView::OpenAddon(AddOn* addon, const char* name)
{
	printf("OpenAddon\n");
	//	extern PicMenuButton *mode;
	Window()->Unlock();
...
void
CanvasView::CloseAddon(AddOn* addon)
{
...
void
CanvasView::Transform(AddOn* addon, uint8 preview)
{
	//	printf ("Transform\n");
	extern Becasso* mainapp;
	extern PicMenuButton* mode;
...
void
CanvasView::Generate(AddOn* addon, uint8 preview)
{
	extern Becasso* mainapp;
	extern PicMenuButton* mode;
...
void
CanvasView::eFilter(BPoint point, uint32 buttons)
{
	while (buttons) {
		if (prev != point) {
...
void
CanvasView::eTransform(BPoint point, uint32 buttons)
{
			...
			if (prev != point) {
				if (!first) {
					...
					if (hascreatednewselection) {
						//						printf ("Restoring previous 2x2 result (s)
						//(cheaply)\n");
...
void
CanvasView::eGenerate(BPoint point, uint32 buttons)
{
...
BBitmap*
CanvasView::canvas(bool do_export)
{
...
void
CanvasView::ConstructCanvas(BBitmap* bitmap, BRect update, bool doselect, bool do_export)
{
	//	clock_t start, end;
	//	start = clock();
	//	extern PicMenuButton *mode;
	...
	if (do_export && !gGlobalAlpha) {
		...
		font.SetSize(size);
		// printf ("Size = %f, ratio = %f, sqrt = %f\n", size, ratio, sqrt (h*h/(ratio*ratio) +
		// w*w));
...
void
CanvasView::Merge(BBitmap* a, Layer* b, BRect update, bool doselect, bool preserve_alpha)
{
	//	clock_t start, end;
	//	start = clock();
	extern bool UseMMX;
...
void
CanvasView::Invalidate(const BRect rect)
{
	clock_t start, end;
	start = clock();
...
void
CanvasView::setScrollBars(BScrollBar* _h, BScrollBar* _v)
{
	hSB = _h;
	vSB = _v;
...
void
CanvasView::SimpleData(BMessage* message)
{
		...
		if (message->what == B_ARCHIVED_OBJECT) {
			...
			for (i = 0; i < numTypes; i++) {
				if (!strcasecmp(types[i], "image/x-becasso"))
					iBecasso = i;
				if (!strcasecmp(types[i], "image/x-png") || !strcasecmp(types[i], "image/png"))
					iPNG = i;
				if (!strcasecmp(types[i], "image/targa") || !strcasecmp(types[i], "image/x-targa"))
					iTarga = i;
			}
			if (iBecasso != -1)
...
void
CanvasView::CopyTarget(BMessage* message)
{
	extern int DebugLevel;
	if (DebugLevel) {
...
void
CanvasView::TrashTarget(BMessage* message)
{
...
void
CanvasView::MouseDown(BPoint point)
{
	//	printf ("MouseDown\n");
...
void
CanvasView::MouseUp(BPoint /*point*/)
{
	//	printf ("MouseUp\n");
	fMouseDown = false;
	if (!entry)
...
void
CanvasView::MouseMoved(BPoint point, uint32 transit, const BMessage* msg)
{
	// clock_t start, end;
	// start = clock();
	point.x /= fScale;
...
void
CanvasView::MouseOrTablet(Position* position)
{
	fMouseDown = true;
...
void
CanvasView::SetupUndo(int32 mode)
{
	//	if (didPreview == 2)
	//	{
...
void
CanvasView::Undo(bool advance, bool menu)
{
	if (menu) {
...
void
CanvasView::ePaste(BPoint /* point */, uint32 buttons)
{
	verbose(1, "ePaste\n");
	extern bool inPaste;
	inPaste = false;
...
void
CanvasView::ePasteM(BPoint point)
{
	extern long pasteX, pasteY;
	extern BBitmap* clip;
	if (!cutbg)
...
void
CanvasView::eDrag(BPoint point, uint32 buttons)
{
	extern BBitmap* clip;
	extern long pasteX, pasteY;
	extern bool inPaste, inDrag;
...
void
CanvasView::SelectAll(bool menu)
{
	if (menu) {
		if (filterOpen || transformerOpen || generatorOpen) {
			beep();
...
void
CanvasView::UndoSelection(bool menu)
...
void
CanvasView::ChannelToSelection(uint32 what, bool menu)
{
	if (menu) {
		if (filterOpen || transformerOpen || generatorOpen) {
			beep();
...
void
CanvasView::SelectionToChannel(uint32 what)
{
	SetupUndo(M_DRAW);
	selection->Lock();
	currentLayer()->Lock();
...
rgb_color
CanvasView::GuessBackgroundColor()
{
	uint32 r, g, b, a, n;
	r = g = b = a = 0;
	currentLayer()->Lock();
...
void
CanvasView::SelectByColor(rgb_color c, bool menu)
{
	if (menu) {
		if (filterOpen || transformerOpen || generatorOpen) {
			beep();
...
void
CanvasView::ColorizeSelection(rgb_color c)
{
	SetupUndo(M_DRAW);
	selection->Lock();
	currentLayer()->Lock();
...
void
CanvasView::InvertSelection(bool menu)
{
	if (menu) {
		if (filterOpen || transformerOpen || generatorOpen) {
			beep();
...
void
CanvasView::FastAddWithAlpha(long x, long y, int strength)
...
//
void
CanvasView::FastBlendWithAlpha(long x, long y, int /* strength */)
// Note: Setup tbits, tbpr, bbits, bbpr, twi, thi, bwi and bhi before you enter here!
...
void
CanvasView::ScrollIfNeeded(BPoint point)
{
...
BRect
CanvasView::makePositive(BRect r)
{
	BRect res;
...
BRect
CanvasView::PRect(float l, float t, float r, float b)
{
	BRect res;
	res.left = min_c(l, r);
	res.right = max_c(l, r);
...
void
CanvasView::WriteAsHex(const char* fname)
{
	BScreen screen;
...
void
CanvasView::ExportCstruct(const char* fname)
{
	BScreen screen;
	bool last = false;
	FILE* f = fopen(fname, "wb");
