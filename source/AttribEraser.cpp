	...
	: AttribView(BRect(0, 0, 126, 102), lstring(23, "Eraser"))
{
	...
	AddChild(type);
	xSlid = new Slider(
		BRect(8, 62, 119, 78), 45, lstring(334, "H Size"), 1, 50, 1, new BMessage('AEhs'));
	ySlid = new Slider(
		BRect(8, 82, 119, 98), 45, lstring(335, "V Size"), 1, 50, 1, new BMessage('AEvs'));
	xSlid->SetValue(16);
	ySlid->SetValue(16);
	fXSize = 16;
	fYSize = 16;
...
status_t
AttribEraser::GetSupportedSuites(BMessage* message)
{
...
void
AttribEraser::MessageReceived(BMessage* msg)
{
	switch (msg->what) {
		case B_GET_PROPERTY:
		{
