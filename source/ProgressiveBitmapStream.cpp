	  ...
	  stop(false)
{
	...
	// Progress bar
	the_bar = new BStatusBar(BRect(10.0, 10.0, 240.0, 50.0), "");
...
ProgressiveBitmapStream::ReadAt(off_t pos, void* buffer, size_t size)
{
	...
	if (from_bitmap > 0) {
		memcpy(static_cast<char*>(buffer) + from_header,
...
ProgressiveBitmapStream::WriteAt(off_t pos, const void* buffer, size_t size)
{
	...
	if (to_bitmap > 0) {
		...
		if (fBar) {
			if (!fBarWin) {
				fBarWin = new BarWindow(BRect(fBarLeftTop, fBarLeftTop + BPoint(250.0, 82.0)),
					fBarTitle, fHeader.dataSize);
			}
			BMessage msg(UPDATE_BAR);
