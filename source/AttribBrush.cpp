	...
	: AttribView(BRect(0, 0, 160, 280), lstring(22, "Brush"))
{
	...
	AddChild(bv);
	ConstructBrush();
	xSlid = new Slider(BRect(4, 160, 154, 176), 60, lstring(300, "X-size"), 1, 40, 1,
		new BMessage('ABxc'), B_HORIZONTAL, 26);
	ySlid = new Slider(BRect(4, 180, 154, 196), 60, lstring(301, "Y-size"), 1, 40, 1,
		new BMessage('AByc'), B_HORIZONTAL, 26);
	aSlid = new Slider(BRect(4, 200, 154, 216), 60, lstring(302, "Angle"), 0, 45, 1,
		new BMessage('ABac'), B_HORIZONTAL, 26);
	sSlid = new Slider(BRect(4, 220, 154, 236), 60, lstring(303, "Strength"), 0, 255, 1,
		new BMessage('ABsc'), B_HORIZONTAL, 26);
	hSlid = new Slider(BRect(4, 240, 154, 256), 60, lstring(305, "Hardness"), 1, 100, 1,
		new BMessage('ABph'), B_HORIZONTAL, 26);
	pSlid = new Slider(BRect(4, 260, 154, 276), 60, lstring(304, "Spacing"), 1, 64, 1,
		new BMessage('ABpc'), B_HORIZONTAL, 26);
	AddChild(xSlid);
	AddChild(ySlid);
	AddChild(aSlid);
	AddChild(sSlid);
...
void
AttribBrush::mkDiagonalBrush(Brush* b, int dir)
{
...
status_t
AttribBrush::GetSupportedSuites(BMessage* message)
{
	message->AddString("suites", "suite/x-sum-becasso-Brush");
...
void
AttribBrush::MessageReceived(BMessage* msg)
{
	switch (msg->what) {
		case B_GET_PROPERTY:
		{
